...
fn k_state(sid: &str) -> String { format!("session:{}:appeal_state", sid) }
fn k_verdict_last(sid: &str) -> String { format!("session:{}:verdict:last", sid) }
fn k_verdict(sid: &str, vid: &str) -> String { format!("session:{}:verdict:{}", sid, vid) }

pub fn load_appeal_state(store: &RocksStore, sid: &str) -> Result<AppealState, AppealError> {
    match store.get_bytes(k_state(sid).as_bytes()).map_err(|e| AppealError::Storage(e.to_string()))? {
        None => Ok(AppealState::Idle),
        Some(b) => serde_json::from_slice(&b).map_err(|e| AppealError::Ser(e.to_string())),
    }
}

pub fn save_appeal_state(store: &RocksStore, sid: &str, st: &AppealState) -> Result<(), AppealError> {
    let b = serde_json::to_vec(st).map_err(|e| AppealError::Ser(e.to_string()))?;
    store.put_bytes(k_state(sid).as_bytes(), &b).map_err(|e| AppealError::Storage(e.to_string()))?;
    Ok(())
}
...
pub fn save_verdict(store: &RocksStore, sid: &str, v: &CouncilVerdict) -> Result<(), AppealError> {
    ...
    store.put_bytes(k_verdict(sid, &v.verdict_id.0).as_bytes(), &b)
        .map_err(|e| AppealError::Storage(e.to_string()))?;
    store.put_bytes(k_verdict_last(sid).as_bytes(), v.verdict_id.0.as_bytes())
        .map_err(|e| AppealError::Storage(e.to_string()))?;
    Ok(())
}

pub fn load_verdict(store: &RocksStore, sid: &str, vid: &str) -> Result<CouncilVerdict, AppealError> {
    let b = store.get_bytes(k_verdict(sid, vid).as_bytes())
        .map_err(|e| AppealError::Storage(e.to_string()))?
...
pub fn on_verdict(_st: AppealState, v: &CouncilVerdict) -> AppealState {
    match v.final_state {
        FinalState::Allowed | FinalState::AllowedWithWarning => AppealState::Closed { verdict_id: v.verdict_id.clone(), final_state: v.final_state.clone() },
        FinalState::RequireConsent | FinalState::RequireUserChoice | FinalState::Blocked => {
            AppealState::AwaitingUser { verdict_id: v.verdict_id.clone(), required: v.next_required.clone() }
        }
    }
}
...
pub fn submit_consent(
    ...
    if vid != v.verdict_id { return Err(AppealError::VerdictMismatch); }
    if let SentinelDecisionKind::Deny = v.sentinel_kind { return Err(AppealError::HardDeny); }

    let c = v.consent.as_ref().ok_or(AppealError::NoActiveAppeal)?;
    if phrase != c.exact_phrase { return Err(AppealError::ConsentMismatch); }
    ...
    Ok(AppealState::Authorized { verdict_id: v.verdict_id.clone(), token, expires_at_ms })
}
